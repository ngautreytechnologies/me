...
import { PortfolioComposer } from '../composition/portfolio-composer.js';

import cssText from '../styles/portfolio-list.css';
import templateHtml from '../templates/portfolio-list.html'; 
    ...
    onConnect() {
        ...
        if (this._pendingTopics && this._pendingTopics.length > 0) {
            ...
            this.renderTopics(this._pendingTopics).catch(err => {
                if (err?.name !== 'AbortError') console.error('[PortfolioList] initial render failed', err);
            });
        } else {
